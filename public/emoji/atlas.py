...
root = ET.Element('svg', {
    'width': str(COLUMNS * SVG_ICON_SIZE),
    'height': str(ROWS * SVG_ICON_SIZE)
})

for i, c in enumerate(CODEPOINTS):
  file = os.path.join('svg', f'{c}.svg')
...
def render(size):
  ...
  os.system(f"inkscape -w {size * COLUMNS} -h {math.ceil(len(CODEPOINTS) / COLUMNS) * size} atlas.svg -o png/{size}x{size}.png")

  print(f"COMPRESSING {size}x{size} ATLAS")
  os.system(f"pngquant --speed 1 --force --output png/{size}x{size}.png png/{size}x{size}.png")
